...
def load_data(data_file):
    ...
    etot = dat[:,3]
    etot -= np.min(etot)
    detot_mev = etot * ry2mev
    return nk1,nk2,nk3,detot_mev

def main(no_tex=False):
    fig = krg_fig_setup(no_tex=no_tex)
    nk1,nk2,nk3,detot_mev = load_data(data_file='TiO2-R-nks_etot_ry.dat')
    plt.plot(nk1/2,detot_mev,'bo-',label=r"\ce{TiO2}-R")
    nk1,nk2,nk3,detot_mev = load_data(data_file='RuO2-R-nks_etot_ry.dat')
    plt.plot(nk1/2,detot_mev,'rx-',label=r"\ce{RuO2}-R")
    thr = 0.0004*13.6056995*1000
    plt.plot([0,np.max(nk1/2)], [thr,thr],'k--',label=r"convergence")
    plt.legend()
    krg_fig_finalize(fig=fig,xlim=[0,7],ylim=[0,100],xlabel=r"$n_{k_1}/2 = n_{k_2}/2 = n_{k_3}/3$",
                     ylabel=r"$\Delta E_{tot}$ (meV)",savefig=True,fname="01-plot-k-point-conv.png")
