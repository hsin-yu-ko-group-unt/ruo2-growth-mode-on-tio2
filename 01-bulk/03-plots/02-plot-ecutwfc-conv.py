...
def load_data(data_file):
    ...
    return ecutwfc,detot_mev

def main(no_tex=False):
    fig = krg_fig_setup(no_tex=no_tex)
    ecutwfc,detot_mev = load_data(data_file='./TiO2-R-ecutwfc_etot.dat')
    plt.plot(ecutwfc,detot_mev,'bo-',label=r"\ce{TiO2}-R")
    ecutwfc,detot_mev = load_data(data_file='./RuO2-R-ecutwfc_etot.dat')
    plt.plot(ecutwfc,detot_mev,'rx-',label=r"\ce{RuO2}-R")
    thr = 0.0004*13.6056995*1000
    plt.plot([0,np.max(ecutwfc)], [thr,thr],'k--',label=r"convergence")
    plt.legend()
    krg_fig_finalize(fig=fig,xlim=[60,140],ylim=[0,100],xlabel=r"$E_{\rm cutwfc}$ (Ry)",
                     ylabel=r"$\Delta E_{tot}$ (meV)",savefig=True,fname="02-plot-ecutwfc-conv.png")
