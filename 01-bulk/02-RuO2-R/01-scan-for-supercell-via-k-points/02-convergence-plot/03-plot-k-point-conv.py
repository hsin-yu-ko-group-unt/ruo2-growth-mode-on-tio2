...
def main(no_tex=True):
    ...
    detot_ev = etot * ry2ev
    x = nk1/2
    plt.plot(x,detot_ev,'b-')
    ax =plt.gca()
    ax.set_xticks(x)
    krg_fig_finalize(fig=fig,xlabel=r"$n_{k_1}/2 = n_{k_2}/2 = n_{k_3}/3$ (Å$^{-1}$)", ylabel=r"$\Delta E_{tot}$ (eV)",savefig=True, fname=  "r-RuO2-k-point-convergence.png", no_display=True)
